<?php
include "../config.php";


class CourC
{
    function addCour($cour)
    {
        $sql = "insert into cour(category, id_seance, teacher, image,type) values (:category, :id_seance, :teacher,  :image,:type)";
        $db = config::getConnexion();
        try {

            $req = $db->prepare($sql);

            $category = $cour->getCategory();
            $id_seance = $cour->getIdSeance();
            $teacher = $cour->getTeacher();
            $image = $cour->getImage();
            $type = $cour->getType();

            $req->bindValue(':category', $category);
            $req->bindValue(':id_seance', $id_seance);
            $req->bindValue(':teacher', $teacher);
            $req->bindValue(':image', $image);
            $req->bindValue(':type', $type);


           $req->execute();
        } catch (Exception $e) {
            echo 'Erreur: ' . $e->getMessage();
        }
    }
    function viewCour()
    {
        $sql = "SELECT * FROM cour";
        $db = config::getConnexion();
        try {

            $liste = $db->query($sql);
            return $liste;
        } catch (Exception $e) {
            die('Erreur: ' . $e->getMessage());
        }
    }
    function viewCourById($id)
    {
        $sql = "SELECT * FROM cour where id=$id";
        $db = config::getConnexion();
        try {

            $liste = $db->query($sql);
            return $liste;
        } catch (Exception $e) {
            die('Erreur: ' . $e->getMessage());
        }
    }
    function viewCourSearch($category)
    {
        $sql = "SELECT * FROM cour where teacher LIKE '$category' ";
        $db = config::getConnexion();
        try {

            $liste = $db->query($sql);
            return $liste;
        } catch (Exception $e) {
            die('Erreur: ' . $e->getMessage());
        }
    }

    function deleteCour($id)
    {

        $sql = "DELETE FROM cour where id= :id";
        $db = config::getConnexion();
        $req = $db->prepare($sql);
        $req->bindValue(':id', $id);
        try {
            $req->execute();
        } catch (Exception $e) {
            die('Erreur: ' . $e->getMessage());
        }
    }
    function updateCour($cour, $id)
    {
        $sql = "UPDATE cour SET category=:category, id_seance=:id_seance, teacher=:teacher, image=:image,type=:type where id=:id";

        $db = config::getConnexion();
        try {
            $req = $db->prepare($sql);

            $category = $cour->getCategory();
            $id_seance = $cour->getIdSeance();
            $teacher = $cour->getTeacher();
            $image = $cour->getImage();
       $type=$cour->getType();

            $req->bindValue(':category', $category);
            $req->bindValue(':id_seance', $id_seance);
            $req->bindValue(':teacher', $teacher);
            $req->bindValue(':image', $image);
            $req->bindValue(':id',$id);
            $req->bindValue(':type', $type);

            $req->execute();

            // header('Location: index.php');
        } catch (Exception $e) {
            echo " Erreur ! " . $e->getMessage();
        }
    }
    function getReview($id){

        $sql = "SELECT * FROM star where id_post = '$id' ";
        $db = config::getConnexion();
        try {

            $liste = $db->query($sql);
            return $liste;
        } catch (Exception $e) {
            die('Erreur: ' . $e->getMessage());
        }
    }
    function getReviewByIP($ip){

        $sql = "SELECT * FROM star where ip= '$ip' ";
        $db = config::getConnexion();
        try {

            $liste = $db->query($sql);
            return $liste;
        } catch (Exception $e) {
            die('Erreur: ' . $e->getMessage());
        }
    }
    function insertIntoRating($thepost,$ip,$therate){
        $sql = "INSERT INTO star (id_post, ip, rate)VALUES('$thepost', '$ip', '$therate')";
        try {

            $req = $db->prepare($sql);



           $req->execute();
        } catch (Exception $e) {
            echo 'Erreur: ' . $e->getMessage();
        }
    }
}
